...
<template>
    <form @submit="onSubmit" class="text-foreground">
                ...
                <AccordionContent class=" md:pl-5 ">
                    <div class="space-y-2 ">
                        <div v-for="(save, index) in item.datas"
                            ...
                            <div class="text-end flex items-center justify-end border-r border-secondary/50 px-2"> <span>{{ save.company || save.grade}}</span></div>
                            <div class="text-end flex justify-end items-center gap-5">
                                <NuxtTime v-if="save.start_date" data-testid="present" :datetime="save.start_date" second="numeric"
                                    month="long" day="numeric" />
                                <Button @click="removeSaved(itemIndex, index)" variant="outlin" class="p-1 h-fit border-none hover:text-secondary" type="button">
                                    <Trash class="size-4" />
                                </Button>
                            </div>
                        </div>
        ...
        <div class="flex justify-between mt-5">
            <nuxt-link  :to="{ name: `app-cv-builder-step-id`, params: { id: ($route.params.id - 1) }, query: { template_id: $route.query.template_id } }" >
